...
export function WeatherCard({ weather, isLoading }: WeatherCardProps) {
  ...
  const weatherDetails = (
    <>
      <p className="order-3 m-0 sm:order-1">
        <time dateTime={new Date(weather.dt * 1000).toISOString()}>
          {formatCityTime(weather.dt, weather.timezone)}
        </time>
      </p>
      <p className="order-2 m-0">Humidity: {weather.main.humidity}%</p>
      <p className="order-1 m-0 sm:order-3">{condition.main}</p>
    </>
  );
    ...
    >
      ...
      <div className="flex justify-between gap-4 sm:block">
        <div>
          ...
          {/* Desktop: Weather Details */}
          <div className="text-muted mt-4 hidden flex-wrap items-baseline gap-x-10 gap-y-1 text-sm sm:flex">
            {weatherDetails}
          </div>
        </div>

        {/* Mobile: Weather Details*/}
        <div className="text-muted flex flex-col justify-end gap-1 text-right text-sm sm:hidden">
          {weatherDetails}
        </div>
      </div>
    </article>
